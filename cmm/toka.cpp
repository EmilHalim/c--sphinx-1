...
void compressoffset(ITOK* thetok)
{
	if (thetok->sib == CODE16)
	{
		if (thetok->rm != rm_d16)
		{
			...
			else if ((int)thetok->number < 128 && (int)thetok->number >= -128)
			{
				thetok->rm |= rm_mod01;
			}
			else
	...
	else
	{
		if (thetok->rm != rm_d32 && thetok->rm != rm_sib)
		{
			...
			if (thetok->number == 0)
			{
				...
				if (thetok->rm == 4 && (thetok->sib & 7) == 5)
				{
					thetok->rm |= rm_mod01;
				}
			}
			else if (thetok->number < 128 && thetok->number >= -128)
			{
				thetok->rm |= rm_mod01;
			}
			else
...
void calcrm(ITOK* itok4, int ttok) //обработка выражения в []
{
	...
	if ((dstok.sib == CODE16 || dstok.sib == (CODE16 + 1)) && dstok.rm != rm_d16)
	{
		razr = r16;
	}
...
loopsw:
		...
		case tk_reg:
			...
			else if (razr == r32 || sizevar != 1 || operand != tk_plus)
			{
				goto runblock;    //16.12.04 22:39
			}
		...
		case tk_minus:
			if (calcnum(&ctok, &cstok, (char*)pstring, &cnum) == 0)
			{
				goto runblock;
			}
...
enumb:
		...
		case tk_reg32:
			...
			if (razr == r16 || (operand != tk_plus && operand != tk_mult) || (idx != -1 && base != -1))
			{
				goto runblock;
			}
...
con1:
		...
		if (ctok == tk_closeblock || ctok == tokens || ctok == tk_eof)
		{
			break;
		}

		if (ctok != tk_plus && ctok != tk_mult)
		{
			goto runblock;
		}
	...
	{
		if (dstok.number < 65536 && dstok.number > -65535)
		{
			cstok.rm = 6;
		}
		else
	...
	else
	{
		...
		if (idx != -1)
		{
			...
			if (idx == 4)
			{
				preerror("ESP cannot be the index register");
			}

			if (base != -1)
			{
				dstok.sib = (zoom << 6) + (idx << 3) + base;
			}
			else
			{
		...
		if (base != -1 || rm != 0)
		{
			cstok.rm |= rm_mod10;    //корекция MOD будет позднее, а сейчас максимум
		}
	}
	...
	if (buf != NULL)
	{
		...
		if ((newreg = CheckIDXReg(buf, sizeel, reg1)) != NOINREG)
		{
			if (newreg != SKIPREG)
			{
				if (razr == r16)
				{
					if (newreg != BX && newreg != DI && newreg != SI && newreg != BP)
					{
						goto noopt;
					}

					if (base0 != -1 && (newreg == BX || newreg == BP))
					{
						goto noopt;
					}
...
noopt2:
			...
			if ((newreg = CheckMassiv(strc->bufstr, sizeel, reg2)) != -1)
			{
				reg2 = newreg;
			}
		}
...
cont2:
	...
	if (ctok->post == 0 && (ctok->flag & f_reloc) == 0)
	{
		compressoffset(ctok);
	}
}
...
int CheckMassiv(char*& buf, int sizeel, int treg, int* idx, int* base, long* num)
{
	...
	if (idx && *idx == -1 && *base == -1)
	{
		...
		do
		{
			...
			if (skipvar)
			{
				for (; startptr < inptr2 - 1; startptr++)
				{
					input[startptr] = ' ';
				}
			}
	...
	if (am32 && sizeel == 1 && tok == tk_reg32 && itok2.type == tp_stopper)
	{
		retcode = itok.number;
	}
	else
	{
		...
		if (treg == AX)
		{
			do_e_axmath(0, (am32 + 1) * 2, &ofsst);
		}
		else
		{
			getintoreg(treg, (am32 + 1) * 2, 0, &ofsst);
		}
	...
	if (sizeel > 1)
	{
		RegMulNum(treg, sizeel, (am32 + 1) * 2, 0, &otok, 0);
	}
...
void nexttok2()
{
	...
	if (tok >= tk_charvar && tok <= tk_doublevar)
	{
		...
		case tp_paramvar:
			...
			//			case tp_ppointer:
			//			case tp_pfpointer:
			if (ptr->fuse == NOTINITVAR && tok2 == tk_assign && (!asmparam))
			{
				warningnotused(itok.name, 4);
			}
...
llq:
		...
		if (calcnumber && CheckConstVar(&itok))
		{
			tok = tk_number;
		}
	...
	{
		if (calcnumber && CheckConstVar(&itok2))
		{
			tok2 = tk_number;
		}
	}
	...
	if (usedirectiv == FALSE && calcnumber == FALSE)while (tok == tk_question)
		{
			directive();
		}
		...
		if (c == '\\')
		{
			...
			case 'x':
				...
				for (i = 0; i < 2; i++)
				{
					...
					if (isdigit(c))
					{
						hold += (unsigned char)(c - '0');
					}
					else if (isxdigit(c))
					{
						hold += (unsigned char)((c & 0x5f) - '7');
					}
					else
					{
						k--;
			...
			default:
				...
				if (isdigit(c))
				{
					...
					for (i = 0; i < 2; i++)
					{
						...
						if (isdigit(c))
						{
							hold = (unsigned char)(hold * 10 + (c - '0'));
						}
						else
						{
							k--;
...
nextstr:
	...
	if (strptr >= (STRLEN - 1) && displaytokerrors)
	{
...
extreg32:
		...
		if (asmparam)
		{
			if ((string4[1] & 0x5f) == 'R')
			{
				if (string4[2] >= '0' && string4[2] <= '7')
				{
					...
					case 'C':
						*tok4 = tk_controlreg;
						return;// check for CR0, CR1, ...

					case 'D':
						*tok4 = tk_debugreg;
						return;//check for DR0, DR1, ...

					case 'T':
						*tok4 = tk_testreg;
						return;//check for TR0, TR1, ...
		...
		if (useasm == FALSE)
		{
			if ((useme = FastSearch(id, idofs, 2, (char*)string4)) != -1)
			{
				*tok4 = useme;
			}
			...
			if (searchlocals(itok4, tok4, string4) == FALSE) //поиск среди локальных меток
			{
				...
				if (*tok4 == tk_endline)
				{
					if (scanlexmode != DEFLEX)
					{
						...
						if (debug)
						{
							printf("1610 tok=%d input=%08X inptr=%08X disp=%d %s %c%40s\n\t%s\n", *tok4, input, inptr, displaytokerrors, itok4->name, cha, input + inptr, ""/*input*/);
						}
				...
				if (*tok4 == tk_number || *tok4 == tk_idasm)
				{
					return;
				}
				...
				if ((*tok4 >= tk_bits && *tok4 <= tk_doublevar) || *tok4 == tk_pointer)
				{
					...
					if (itok4->post == DYNAMIC_POST) 	//преобразовать динамическую локальную в локальную
					{
						...
						if (am32 == FALSE && (postsize + itok4->size) > 0xFFFFL)
						{
							tobigpost();
						}
		...
		case tk_postptr:
			*tok4 = tk_number;
			itok4->number = postsize;
			break;

		case tk_line:
			*tok4 = tk_number;
			itok4->number = linenumber;
			break;
			...
			if (findofset == FALSE)
			{
						...
						(tteg = FindTeg(TRUE, itok4->name)) != NULL) && (cha == '.' || precha == '.'))
				{
					...
					if (displaytokerrors && (*tok4 == tk_proc || *tok4 == tk_declare ||
											 *tok4 == tk_undefproc) && (itok4->flag & f_static) == 0)
					{
						unknownobj(itok4->name);
					}
				}
			}
...
yesid:
		...
		if ((*tok4 >= tk_bits && *tok4 <= tk_doublevar) || *tok4 == tk_pointer)
		{
			...
			if (cha == '[')
			{
				calcrm(itok4, *tok4);    //обработка выражения в []
			}
		}
	...
	else if (isdigit(cha)) //числа
	{
		...
		*tok4 = tk_number;
		...
		case '%':
			...
			if (cha == '-')
			{
				*tok4 = tk_modminus;    //остаток от деления
			}
			else
			{
				*tok4 = tk_mod;
		...
		case '>':
			...
			case '>':
				...
				if (cha == '=')
				{
					*tok4 = tk_rrequals;    //сдвиг вправо с присвоением
				}
				else
				{
					whitespace();
		...
		case '<':
			...
			case '<':
				...
				if (cha == '=')
				{
					*tok4 = tk_llequals;    //сдвиг влево с присвоением
				}
				else
				{
					whitespace();
...
structofs:
						...
						(tteg = FindTeg(TRUE, itok4->name)) != NULL)/*&&cha=='.'*/)
				{
					...
					if (*tok4 == tk_proc)
					{
						goto procofs;    //04.10.04 14:25
					}

					if (*tok4 == tk_declare)
					{
						goto undefofs;    //24.09.04 00:33
					}
...
undefofs:
		...
		case ';':
			itok4->type = tp_stopper;
			*tok4 = tk_semicolon;
			break;

		case ')':
			itok4->type = tp_stopper;
			*tok4 = tk_closebracket;
			break;
...
void NewMod(int numipar)
{
	...
	if (cha == '(')
	{
		...
		if (cha != ')')
		{
			...
			for (i = inptr, ns = 1; ns > 0; i++) 	//поиск параметров
			{
				...
				case ',':
					if (ns == 1)
					{
						...
						if (!buf)
						{
							buf = (char*)MALLOC(nsize + 1);
						}
						else
						{
							buf = (char*)REALLOC(buf, size + nsize + 1);
						}
			...
			{
				buf = (char*)REALLOC(buf, size + nsize);
			}
		...
		if (numipar > numpar)
		{
			missingpar(itok.name);
		}
		else if (numipar < numpar)
		{
			extraparam(itok.name);
		}
...
//поиск в дереве переменых
{
	...
	for (ptr = fptr; ptr != NULL;)
	{
		...
		{
			if (scanlexmode == RESLEX && ptr->rectok != tk_number && ptr->rectok != tk_string)
			{
				break;
			}
			...
			*tok4 = ptr->rectok;
			...
			if (*tok4 == tk_macro)
			{
				...
				if (scanlexmode == DEFLEX || scanlexmode == DEFLEX2)
				{
					*tok4 = tk_id;
				}
			...
			else if (ptr->newid)
			{
				...
				case tk_string:
					...
					if (displaytokerrors && itok4->rm == 1 && ptr->sbuf)
					{
						strcpy((char*)string3, ptr->sbuf);
					}
				...
				case tk_structvar:
					...
					if (ptr->recpost == DYNAMIC_POST)
					{
						...
						if (am32 == FALSE && (ptr->recsize + postsize) > 0xFFFFL)
						{
							tobigpost();
						}
					...
					else if (ptr->recpost == DYNAMIC_VAR)
					{
						itok4->post = ptr->recpost = USED_DIN_VAR;
					}
...
int searchtree(ITOK* itok4, int* tok4, unsigned char* string4)
{
	...
	if (skipfind == FALSE)
	{
		if (staticlist)
		{
			retval = searchtree2(staticlist, itok4, tok4, string4);
		}

		if (!retval)
		{
			retval = searchtree2(treestart, itok4, tok4, string4);
		}
...
void AddDynamicList(idrec* ptr)
{
	...
	for (int i = 0; i < countDP; i++)if (ptr == DynamicList[i])
		{
			return;
		}
	...
	if (ptr != NULL)
	{
		...
		if (sdp_mode == FALSE && (ptr->flag & f_export) != 0 && tok == tk_proc)
		{
			...
			else
			{
				...
				{
					if ((cmpname = strcmp(ptr->recid, (lexport + i)->name)) <= 0)
					{
						break;
					}
				}
		...
		if (!(ptr->flag & f_extern))
		{
			...
			else if (tok == tk_proc)
			{
				...
				if (itok.segm == DYNAMIC_USED)
				{
					...
					{
						strcpy((char*)string, (char*)ptr->newid);
					}
...
void docalls2()
{
	...
	for (unsigned int i = 0; i < totalmodule; i++)
	{
		docals((startfileinfo + i)->stlist);
	}
...
elementteg* FindOneDestr(structteg* searcht)
{
	...
	for (i = 0; i < searcht->numoper; i++)
	{
		...
		if ((bazael + i)->tok == tk_proc && (ptrs->flag & fs_destructor) != 0)
		{
			return (bazael + i);
		}
	}
	...
	for (i = 0; i < searcht->numoper; i++)
	{
		...
		{
			if ((retrez = FindOneDestr((structteg*)ptrs)) != NULL)
			{
				break;
			}
		}
...
void Destructor(idrec* ptrs)
{
	...
	{
		if ((bazael = FindOneDestr(tteg)) == NULL)
		{
			preerror("destructor not defined");
		}
	}

	if (ptrs->recsize)
	{
		CreatParamDestr(ptrs);
	}
	else
	{
		structadr.sib = THIS_ZEROSIZE;
	}
...
void dodelete()
{
	...
	if (tok == tk_structvar)
	{
		...
		else if (itok.rec->newid && AskUseDestr((structteg*)itok.rec->newid))
		{
			Destructor(itok.rec);
		}

		//		}
		if (oinptr != tk_semicolon)
		{
			nexttok();
		}
	}
...
int AskUseDestr(structteg* searcht)
{
	...
	for (int i = 0; i < searcht->numoper; i++)
	{
		...
		{
			if (AskUseDestr((structteg*)(bazael + i)->rec))
			{
				return TRUE;
			}
		}
			...
			if (ptrs->rec.rectok == tk_structvar)
			{
				if ((ptrs->rec.flag & fs_destructor) != 0 || AskUseDestr((structteg*)ptrs->rec.newid))
				{
					if (!calldestruct)
					{
						...
						if (ptrs->rec.recsize == 0 || returntype == tk_void)
						{
							zerosize = TRUE;
						}
						else
						...
						structteg* subteg)
{
	...
	for (unsigned int i = 0; i < searcht->numoper; i++)
	{
		if ((bazael + i)->tok == tk_baseclass)
		{
			if (!numclass)
			{
				listclass = (void**)MALLOC(sizeof(void**));
			}
			else
			{
				listclass = (void**)REALLOC(listclass, sizeof(void**) * (numclass + 1));
			}
...
int CallDestructor(structteg* searcht)
{
	...
	for (int i = 0; i < searcht->numoper; i++)
	{
		...
		if ((searcht->flag & fs_destructor) == 0)
		{
			if ((bazael + i)->tok == tk_baseclass)
			{
				if (CallDestructor((structteg*)ptrs))
				{
					return TRUE;
				}
			}
...
//поиск локальных переменых связаного списка
{
				...
				*tok4 = ptr->rec.rectok;
				...
				if (displaytokerrors)
				{
					if (mapfile)
					{
						if (ptr->li.count == 0)
						{
							ptr->li.usedfirst = linenumber;
						}
						else
						{
							ptr->li.usedlast = linenumber;
						}
				...
				if (ptr->rec.type == tp_gvar)
				{
					...
					if (ptr->rec.recpost == DYNAMIC_VAR)
					{
						ptr->rec.recpost = USED_DIN_VAR;
					}
				...
				if (ptr->rec.type == tp_paramvar || ptr->rec.type == tp_localvar)
				{
					if (ESPloc && am32)
					{
						...
						{
							itok4->number += localsize + addESP;
						}
						else
						{
							itok4->number += -4 + addESP;
						}
					}
					else
					{
	...
	if (searchteg)
	{
		...
		if (cha == ':' && cha2 == ':')
		{
			...
			if ((subteg = FindTeg(TRUE, itok4->name)) == NULL && displaytokerrors)
			{
				undefclass(itok4->name);
			}
		...
		if ((bazael = FindClassEl(searchteg, string4, &addofs, subteg)) != NULL)
		{
			*tok4 = bazael->tok;
			...
			if (*tok4 == tk_proc || *tok4 == tk_pointer)
			{
				...
				if (*tok4 == tk_proc && ptr->newid != NULL)
				{
					strcpy((char*)string4, ptr->newid);
				}
...
notnum:
			...
			if (displaytokerrors)
			{
				...
				if (strinf.bufstr != NULL)
				{
					free(strinf.bufstr);    //internalerror("Previous block was not used");
				}
	...
	if (cha == '.')
	{
		...
		do
		{
			...
			if (cha == '.')
			{
				searchtree2(definestart, &cstok, &i, (unsigned char*)cstring);
			}
		}
		while (i == tk_endline);
		...
		if (cha == ':')
		{
			...
			if ((subteg = FindTeg(TRUE, cstok.name)) == NULL && displaytokerrors)
			{
				undefclass(cstok.name);
			}
		...
		if ((bazael = FindClassEl(tteg, (unsigned char*)cstok.name, &addofs, subteg)) == NULL)
		{
			if (displaytokerrors)
			{
				unknownstruct(cstok.name, tteg->name);
			}
		...
		*tok4 = bazael->tok;
		...
		if ((itok4->flag & f_extern) != 0)
		{
			itok4->number += ((bazael->ofs + cnum) << 16);
		}
		else
		{
			itok4->number += bazael->ofs + cnum;
		}
		...
		if (*tok4 == tk_proc || *tok4 == tk_pointer)
		{
			...
			if (*tok4 == tk_proc)
			{
				...
				if (structadr.post == LOCAL)
				{
					...
					if (tteg->size == 0)
					{
						structadr.sib = THIS_ZEROSIZE;    //14.11.05 13:51
					}
				}
			...
			*tok4 = ptr->rectok;
			...
			if (displaytokerrors)
			{
				...
				if (*tok4 == tk_proc && itok4->segm == DYNAMIC)
				{
					AddDynamicList(ptr);
				}

				if (*tok4 != tk_proc && *tok4 != tk_undefproc && *tok4 != tk_declare)
				{
					thisundefined(ptr->recid, FALSE);
				}
			}
...
void dosizeof(ITOK* itok4)	//опр значение sizeof
{
	...
	else
	{
		...
		case tk_doublevar:
		case tk_interruptproc:
			if (cstok.size == 0)if (displaytokerrors)
				{
					preerror("unknown size");
				}
		...
		default:
			...
			if (strcmp("file", (char*)string2) == 0)
			{
				...
				if (i == tk_string)
				{
					...
					if (stat((char*)string3, &statbuf) != 0)
					{
						unableopenfile((char*)string3);
					}
					else
					{
						itok4->number = statbuf.st_size;
					}
				}
				else
				{
			...
			{
				preerror("illegal use of sizeof");
			}
...
void ofsstr(int* tok4, ITOK* itok4)
{
	...
	if (strinf.bufstr == NULL && itok4->post == TRUE)
	{
		itok4->number += (tteg->size * (itok4->rm));
	}
		...
		else
		{
			if (displaytokerrors)
			{
				if ((newreg = CheckIDXReg(strinf.bufstr, strinf.size, idxregs[0])) != NOINREG)
				{
					...
					{
						if (am32 == FALSE && newreg != DI && newreg != BX)
						{
							goto noopt2;
						}
...
*/
{
	...
	for (curptr = undefoffstart;; curptr = curptr->next)
	{
		if (strcmp(curptr->name, ostring) == 0) 	//ранее уже обращались к ней
		{
			...
			{
				curptr->pos = (IOFS*)REALLOC(curptr->pos, sizeof(IOFS) * (curptr->num + 1));
			}
...
int CheckUseAsUndef(unsigned char* name)
{
	...
	{
		return 0;    //не было обращений к undef
	}
...
int FindOff(unsigned char* name, int base)	//поиск ссылок на текущее имя
{
	...
	for (curptr = undefoffstart;; curptr = curptr->next)
	{
		if (strcmp(curptr->name, (char*)name) == 0) 	//нашли
		{
			for (int i = 0; i < curptr->num; i++)
			{
				...
				else
				{
					if ((segm & 1) == 0 || modelmem == TINY) 	//в сегменте кода
					{
						...
						if (am32 == FALSE)
						{
							*(unsigned short*)&output[ofs] += (unsigned short)valofs;
						}
						else
						{
							*(unsigned long*)&output[ofs] += valofs;
						}
					}
					else
					{
						if (am32 == FALSE)
						{
							*(unsigned short*)&outputdata[ofs] += (unsigned short)(base == CS ? outptr : outptrdata);
						}
						else
						{
							*(unsigned long*)&outputdata[ofs] += (unsigned long)(base == CS ? outptr : outptrdata);
						}
					}
			...
			if (undefoffstart->next == NULL)
			{
				undefoffstart = NULL;
			}
			else if (undefoffstart == curptr)
			{
				undefoffstart = curptr->next;
			}
			else
			{
...
unsigned long getsizebit(int size)
{
	...
	if (num > (unsigned int)(size * 8))
	{
		preerror("Bit field to large");
	}
...
void AddTegToTree(structteg* newteg, int Global)
{
	...
	if (tteg == NULL)
	{
		(Global == TRUE ? tegtree : ltegtree) = newteg;
	}
	else
	{
...
int IsClass(structteg* searcht)
{
	...
	for (int i = 0; i < searcht->numoper; i++)
	{
		...
		{
			if ((bazael + i)->rec && ((bazael + i)->rec->flag & f_classproc))
			{
				return TRUE;
			}
		}
...
struct structteg* CreatTeg(int Global, int useunion, int noname)	//создать новый тег
{
	...
	if (tok == tk_colon)
	{
		...
		if (useunion)
		{
			preerror("union cannot have a base type");
		}
	}
		...
		case tk_union:
			...
			if ((tteg = CreatTeg(Global, TRUE, i)) != NULL)
			{
				if (tok == tk_semicolon)
				{
					if (numel == 0)
					{
						bazael = (struct elementteg*)MALLOC(sizeof(struct elementteg) * tteg->numoper);
					}
					else
					{
						bazael = (struct elementteg*)REALLOC(bazael, sizeof(struct elementteg) * (numel + tteg->numoper));
					}
					...
					else if (unionsize < (unsigned int)(tsize))
					{
						unionsize = tsize;
					}
...
locvar:
			...
			do
			{
				...
				if (tok == tk_colon)
				{
					...
					if (numt == 0)
					{
						numt = size * 8 - bitofs % (size * 8);    //(am32==0?16:32)-bitofs%(am32==0?16:32);
					}
...
dproc2:
				...
				if (tok != tk_ID && tok != tk_id)
				{
					...
					else
					{
						...
						if (tok == tk_openbracket || type == pointer)
						{
							goto dproc;
						}
				...
				else
				{
					...
					if (tok2 == tk_openbracket && utestInitVar == FALSE)
					{
						if (tok == tk_id)
						{
							oflag |= (comfile == file_w32 ? tp_stdcall : tp_pascal);    //тип проц по умолчанию
						}
						else
						{
							oflag = tp_fastcall;
					...
					if (tok == tk_colon)
					{
						...
						if (numt == 0)
						{
							preerror("Bit fields must contain at least one bit");
						}
...
dproc:
						...
						if (orm == tokens)
						{
							orm = am32 == FALSE ? tk_word : tk_dword;    //tk_void;
						}
...
endelteg:
	...
	if (bitofs && useunion == FALSE)
	{
		ssize += (bitofs + 7) / 8;
	}
	else if (bitsize && ((unsigned int)((bitsize + 7) / 8) > unionsize))
	{
		unionsize = (bitsize + 7) / 8;
	}
	...
	if (isdestr == FALSE && isbase > 1)
	{
		...
		{
			if (GetFirstDestr((structteg*)(bazael + i)->nteg))
			{
				j++;
			}
		}
...
struct structteg* FindTeg(int Global, char* name)	//найти тег
{
	...
	while (tteg && (i = strcmp(tteg->name, name)) != 0)
	{
		tteg = (i < 0 ? tteg->left : tteg->right);
	}
 ...
 --------------------------------------------------*/
{
	...
	for (unsigned int j = 0; j < numel; j++)
	{
		...
		for (unsigned int c = 0; c < tteg->numoper; c++)
		{
			if ((elem + c)->ofs)
			{
				outptrdata = startstruct + (elem + c)->ofs;
			}
 ...
 --------------------------------------------------*/
{
	...
	for (;;)
	{
		...
		if ((elem + ttype)->rec && ((elem + ttype)->rec->flag & f_static))
		{
			continue;
		}
		...
		if (dbg & 2)
		{
			AddDataLine((char)GetVarSize(type)/*,variable*/);
		}
...
loopsw:
		...
		case tk_number:
			...
			else if (type == tk_floatvar)
			{
				hold = doconstfloatmath();
			}
			else if (type == tk_doublevar)
			{
				hold = doconstdoublemath();
			}
			else if (type == tk_qwordvar)
			{
...
strl3:
		...
		case tk_string:
			if (type == tk_bytevar || type == tk_charvar)
			{
				...
				if ((unsigned int)itok.number < (elem + ttype)->numel)
				{
					i = itok.number;
				}
				else
			...
			{
				if (type == tk_intvar || type == tk_wordvar)
				{
					dwordvalexpected();
				}
...
void InitStruct2(unsigned int flag, structteg* tteg)	//инициализировать глобальную структуру
{
	...
	while (tok != tk_eof && done == 0)
	{
		...
		case tk_ID:	//инициализировать структуру
			...
			if (ptr == NULL)
			{
				((flag & f_static) == 0 ? treestart : staticlist) = newrec;    //начало дерева
			}
			...
			else
			{
				...
				if (CheckUseAsUndef((unsigned char*)newrec->recid) == 0 && dynamic_flag)
				{
					dynamic = TRUE;
				}
		...
		case tk_number:
			...
			if (strinf.bufstr == NULL)
			{
				...
				else
				{
					...
					else if (chip > 2 && sized != 4 && (num % 4) == 0 && itok.number == 0)
					{
						sized = 4;
					}
				}
			...
			else
			{
				...
				{
					MovRegNum(sized, 0, itok.number, EAX);
				}
			}
...
fillstr:
		...
		case tk_structvar:
			...
			if (usenumstr == FALSE)
			{
				num *= tteg->size;    //ptrs->recrm;	//не указан номер структуры - значит все
			}
...
int RegToRM(int number, int tok4)
{
	...
	if (displaytokerrors)
	{
		if (am32 == FALSE && tok4 == tk_reg32)
		{
			regBXDISIBPexpected();
		}
		else if (am32 && tok4 == tk_reg)
		{
			reg32expected();
		}
	}
...
void GetTypeVar(int* tok4)
{
	...
	if (*tok4 == tk_signed)
	{
		if (i == tk_char || i == tk_int || i == tk_long)
		{
			*tok4 = i;
			...
			if (*tok4 == tk_long)
			{
				...
				else if (i != tk_int && i != 0 && displaytokerrors)
				{
					unknowntype();
				}
			}
		...
		else if (i == tk_short)
		{
			...
			*tok4 = tk_int;
			...
			if (i != 0 && i != tk_int && displaytokerrors)
			{
				unknowntype();
			}
		}
	...
	else if (i == tk_int && (*tok4 == tk_long || *tok4 == tk_short))
	{
		*tok4 = i;
	}
	...
	else if (*tok4 == tk_int && (!(idasm == TRUE && i == tk_number)) && am32)
	{
		*tok4 = tk_long;
	}
	else if (i == tk_long && *tok4 == tk_long)
	{
		*tok4 = tk_qword;
	}

...
nexpardll:
			...
			case c_mer:
				...
				if ((maxerrors = getnumber((unsigned char*)ptr)) == 0)
				{
					maxerrors = 16;
				}
			...
			case c_nw:
				...
				{
					wact[len - 1].usewarn = (unsigned char)0 ^ neg;
				}
			...
			case c_we:
				...
				if (len > 0 && len <= WARNCOUNT)
				{
					if (neg)
					{
						wact[len - 1].fwarn = warningprint;
					}
					else
					{
						wact[len - 1].fwarn = preerror3;
					}
				}
...
void LoadIni(char* name)
{
	...
	for (;;)
	{
		if (fgets(m1, 255, inih) == NULL)
		{
			break;
		}

		if (SelectComand(m1, 0) == c_end)
		{
			BadCommandLine(m1);
		}
	}
...
void doposts()
{
	...
	if (am32 == 0 && (MAXDATA - outptrdata) < postsize)
	{
		preerror("post variable size exceeds size left in data segment");
	}
...
void GetMemExeDat()
{
	if (outputdata == output && outputdata != 0)
	{
...
savecode:
		...
		if (fwrite(output + startptr, outptr - startptr, 1, hout) != 1)
		{
			goto errwr;
		}
		...
		{
			if (fwrite(outputdata, outptrdata, 1, hout) != 1)
			{
				goto errwr;
			}
		}
...
void MakeExeHeader(EXE_DOS_HEADER* exeheader)
{
	...
	else if (comfile == file_d32)
	{
		...
		{
			exeheader->maxmem = (unsigned short)0xFFFF;    // max-paragraphs
		}
	}
	else
	{
...
void warnunused(struct idrec* ptr)
{
	...
	if (ptr != NULL)
	{
		if (ptr->count == 0 && startupfile != ptr->file)
		{
			...
			{
			case tk_proc:
				if (ptr->recsegm != NOT_DYNAMIC || strcmp(ptr->recid, mesmain) == 0)
				{
					break;
				}
			...
			case tk_qword:
			case tk_double:
				if (i <= 1 && (ptr->recpost == DYNAMIC_VAR || ptr->recpost == DYNAMIC_POST))
				{
					break;
				}
...
void WarnUnusedVar()//предупреждения о неиспользованных процедурах и переменных
{
	...
	{
		warnunused((startfileinfo + i)->stlist);
	}
}
...
void addinitvar()
{
	...
	{
		if (alignword || optimizespeed)
		{
			AlignCD(DS, chip > 5 ? 16 : 4);
		}
		...
		for (i = 0; i < posts; i++)
		{
			if ((postbuf + i)->type == POST_FLOATNUM)
			{
				if (am32)
				{
					*(unsigned long*)&output[(postbuf + i)->loc] = outptrdata + (postbuf + i)->num;
				}
				else
				{
					*(unsigned short*)&output[(postbuf + i)->loc] = (unsigned short)(outptrdata + (postbuf + i)->num);
				}

				if (FixUp)
				{
					(postbuf + i)->type = FIX_VAR32;
				}
				else
				{
					killpost(i--);
		...
		for (i = 0; i < numfloatconst; i++)
		{
			if (dbg & 2)
			{
				...
				{
					sprintf((char*)string, "const float %f", (floatnum + i)->fnum);
				}
				else
				{
					sprintf((char*)string, "const double %f", (floatnum + i)->dnum);
				}
	...
	for (i = 0; i < (unsigned int)numswtable; i++) 	//создать и вставить таблицы switch
	{
		...
		if (dbg & 2)
		{
			AddDataNullLine((char)swt->type, "switch table address");
		}
		...
		{
			*(unsigned long*)&output[swt->ptb] = outptrdata;
		}
		...
		if (swt->mode == 2)
		{
			if (dbg & 2)
			{
				AddDataNullLine((char)swt->razr, "switch table value");
			}
			...
			{
				*(unsigned long*)&output[swt->ptv] = outptrdata;
			}
		}
		...
		for (int jj = 0; jj < swt->numcase; jj++)
		{
			...
			if ((swt->info + jj)->type == singlcase)
			{
				if (swt->mode == 1)
				{
					if (am32 == FALSE)
					{
						*(unsigned short*)&outputdata[oline + j * 2] = (unsigned short)val;
					}
					else
					{
						*(unsigned long*)&outputdata[oline + j * 4] = val;
					}
				}
				else
				{
					if (am32 == FALSE)
					{
						*(unsigned short*)&outputdata[oline + ii * 2] = (unsigned short)val;
					}
					else
					{
						*(unsigned long*)&outputdata[oline + ii * 4] = val;
					}
	...
	for (i = 0; i < posts; i++)
	{
		if ((postbuf + i)->type == DIN_VAR || (postbuf + i)->type == DIN_VAR32)
		{
			...
			if (FixUp)
			{
				(postbuf + i)->type = (unsigned short)((postbuf + i)->type == DIN_VAR ? FIX_VAR : FIX_VAR32);
			}
			else
			{
				killpost(i--);
...
void setdindata(idrec* ptr, int i)
{
	...
	if (alignword)
	{
		if (ptr->rectok == tk_structvar)
		{
			alignersize += AlignCD(DS, 2);    //выровнять
		}
		else
		{
			alignersize += AlignCD(DS, GetVarSize(ptr->rectok));
		}
	}
	...
	if ((postbuf + i)->type == DIN_VAR)
	{
		*(unsigned short*)&output[(postbuf + i)->loc] += (unsigned short)(outptrdata);
	}
	else
	{
		*(unsigned long*)&output[(postbuf + i)->loc] += outptrdata;
	}
	...
	if (tok == tk_structvar)
	{
		datasize += initstructvar((structteg*)ptr->newid, ptr->recrm);
	}
	else
	{
		long type, ssize;
		...
		else if (tok == tk_pointer)
		{
			...
			if (am32 == FALSE && ((itok.flag & f_far) == 0))
			{
				ssize = 2;
			}
			else
...
{
	if (numundefclassproc == 0)
	{
		undefclassproc = (idrec**)MALLOC(sizeof(idrec**));
	}
	else
	{
		undefclassproc = (idrec**)REALLOC(undefclassproc, sizeof(idrec**) * (numundefclassproc + 1));
	}
